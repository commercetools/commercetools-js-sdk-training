(function (global, factory) {
	...
	(factory((global.CommercetoolsSdkMiddlewareAuth = {}),global.require$$0,global.http,global.https,global.zlib,global.require$$1,global.require$$0$1,global.require$$1$1,global.util));
...
function buildRequestForClientCredentialsFlow(options) {
  ...
  var defaultScope = MANAGE_PROJECT + ':' + options.projectKey;
  ...
  return { basicAuth: basicAuth, url: url, body: body };
}
...
function buildRequestForPasswordFlow(options) {
  ...
  var scopeStr = scope ? '&scope=' + scope : '';

  var basicAuth = Buffer.from(clientId + ':' + clientSecret).toString('base64');
  // This is mostly useful for internal testing purposes to be able to check
  // other oauth endpoints.
  var oauthUri = options.oauthUri || '/oauth/' + pKey + '/customers/token';
  var url = options.host.replace(/\/$/, '') + oauthUri;
  // eslint-disable-next-line max-len
  var body = 'grant_type=password&username=' + username + '&password=' + password + scopeStr;
...
function buildRequestForRefreshTokenFlow(options) {
  ...
  var oauthUri = options.oauthUri || '/oauth/token';
  var url = options.host.replace(/\/$/, '') + oauthUri;
  var body = 'grant_type=refresh_token&refresh_token=' + options.refreshToken;
...
function executeRequest(_ref, next) {
  var url = _ref.url,
  ...
  fetch(url, {
    ...
    if (res.ok) return res.json().then(function (_ref2) {
      ...
      // Cache new token
      tokenCache.set({ token: token, expirationTime: expirationTime, refreshToken: refreshToken });
...
function authMiddlewareBase(_ref3, next, userOptions) {
  ...
  pendingTasks.push({ request: request, response: response });
  ...
  if (tokenObj && tokenObj.refreshToken && (!tokenObj.token || tokenObj.token && Date.now() > tokenObj.expirationTime)) {
    executeRequest(_extends({}, buildRequestForRefreshTokenFlow(_extends({}, userOptions, {
